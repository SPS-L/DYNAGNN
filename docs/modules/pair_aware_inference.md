...
Expects `model_type == "pair_aware_gine"` and fields including: `model_state_dict`, `hparams`, vocab sizes/maps, `selected_output`, `cuts`, log-KPI mean/std, `epsilon`, `inactive_gate_threshold`, and `flag_gate_threshold` (legacy checkpoints may store `gate_threshold` or `class5_gate_threshold`).
