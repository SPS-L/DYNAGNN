...
"""Train a direct pair-aware six-class Spower GNN.
...
from modules.gat_spower_pair_aware_six_class import (  # noqa: E402
    ACTIVITY_CLASSES,
    NUM_CLASSES,
    PairAwareHParams,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Direct pair-aware six-class Spower GNN")
    ...
    parser.add_argument("--scenario-name", default="Pair-aware direct six-class Spower GNN")
...
def configure_logger(output_dir: Path) -> logging.Logger:
    logger = logging.getLogger("dynagnn_spower_pair_aware_six_class")
...
def resolve_paths(data_path: Path, graph_dir: Path | None) -> dict[str, Path]:
    ...
    return {
        ...
        "kpi_spower": first_existing(
            [data_path / "KPI" / "KPI_spower.csv", data_path / "KPI_spower.csv"],
            "KPI_spower.csv",
        ),
        "disc_spower": first_existing(
            [
                data_path / "Disconnections" / "DISC_spower.csv",
                data_path / "DISC" / "DISC_spower.csv",
                data_path / "Dataset" / "DISC_spower.csv",
                data_path / "DISC_spower.csv",
            ],
            "DISC_spower.csv",
        ),
...
def attach_pair_aware_tensors(
    graph_dataset: list,
    *,
    kpi_spower: pd.DataFrame,
    disc_spower: pd.DataFrame,
    epsilon: float,
) -> dict:
    ...
    for col in ("OP", "Contingency"):
        if col not in kpi.columns:
            raise KeyError(f"KPI_spower.csv is missing {col}")
    ...
    for data in graph_dataset:
        ...
        for key, meta in (metadata.get("node_metadata", {}) or {}).items():
            ...
            # Use the exact Spower supervision mask built from dynamic generator nodes.
            if bool(data.gen_node_mask[idx].item()):
                if node_id not in kpi.columns:
                    raise KeyError(f"KPI_spower.csv is missing generator column {node_id!r}")
                ...
                if node_id not in disc.columns:
                    raise KeyError(f"DISC_spower.csv is missing generator column {node_id!r}")
        ...
        y_gen = data.y_class[data.gen_node_mask]
        structural_gen = structural_class5_mask[data.gen_node_mask]
        stats["structural_class5_targets"] += int(structural_gen.sum().item())
        stats["class5_label_mask_mismatches"] += int(((y_gen == 5) != structural_gen).sum().item())
...
def standardize_log_targets(train_graphs: list, all_splits: Sequence[list]) -> tuple[float, float]:
    ...
    for data in train_graphs:
        mask = data.gen_node_mask.bool() & (data.y_class >= 0) & (data.y_class < ACTIVITY_CLASSES)
...
def main() -> None:
    ...
    kpi_spower = filter_frame(pd.read_csv(paths["kpi_spower"], low_memory=False), allowed_ops)
    disc_spower = filter_frame(pd.read_csv(paths["disc_spower"], low_memory=False), allowed_ops)
    ...
    # Expose Spower labels before tensor attachment so class-5 counts are available.
    for data in graphs:
        data.y_class = data.y_spower
    append_electrical_distance_feature(
        graphs,
    ...
    attachment = attach_pair_aware_tensors(
        graphs, kpi_spower=kpi_spower, disc_spower=disc_spower, epsilon=args.epsilon
    )
    ...
    if int(attachment["class5_label_mask_mismatches"]) != 0:
        raise RuntimeError(
            "DISC_spower structural mask does not match class-5 labels: "
