...
"""Pair-aware direct six-class Spower GNN for DYNAGNN.
...
- classes 0..4: generator apparent-power KPI activity levels;
...
class PairAwareDirectSpowerModel(nn.Module):
    ...
    def forward(self, data) -> dict[str, torch.Tensor]:
        ...
        target_mask = data.gen_node_mask.bool()
    ...
    *,
    ...
    with torch.set_grad_enabled(train_mode):
        for data in loader:
            ...
            output = model(data)
            y_all = data.y_class[data.gen_node_mask].long()
            log_target_all = data.y_log_kpi_std[data.gen_node_mask]
            structural_all = data.structural_class5_mask[data.gen_node_mask].bool()
            ...
            if collect_predictions:
                ...
                target_graph_all = data.batch[data.gen_node_mask].detach().cpu().numpy()
                target_token_all = data.node_token[data.gen_node_mask].detach().cpu().numpy()
...
def _compute_class_weights(loader, mode: str, device: torch.device) -> Optional[torch.Tensor]:
    ...
    for data in loader:
        y = data.y_class[data.gen_node_mask]
...
def run_pair_aware_training(
    *,
    ...
    training_dir = output_dir / "training" / "spower_pair_aware_six_class_gnn"
